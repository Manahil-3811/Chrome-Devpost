...
async function loadAndDisplayTabs() {
  ...
  try {
    ...
    if (tabs.length === 0) {
      tabsResultDiv.innerHTML = '<p class="no-tabs">No tabs found in this window.</p>';
      return;
    }
    ...
    if (groups.length > 0) {
      groups.forEach(group => {
        ...
        groupTabs.forEach(tab => {
          const favicon = tab.favIconUrl || 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="%235165ea" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>';
...
document.getElementById("save-group")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (typeof chrome.tabGroups === 'undefined') {
      alert("Chrome Tab Groups API is not available. This requires Chrome 89+ and the 'tabGroups' permission in manifest.json");
      return;
    }
